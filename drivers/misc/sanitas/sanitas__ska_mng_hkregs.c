...
#define DRIVER_NAME "ska_mng_hkregs"

struct SKA_MNG_HKREGS_deviceData
{
	struct device *dev;
	void __iomem* base;
...
{
	const Register* reg = to_register_dev_attr(attr)->reg;
	return Registers_List(SKA_MNG_HKREGS_registers, buf, PAGE_SIZE);
}
		...
		char *buf)
{
	...
	return Registers_MemoryDump32(data->base, SKA_MNG_HKREGS_registers, buf, PAGE_SIZE);
}
static DEVICE_ATTR(DumpRegistersMap, S_IRUGO, dumpRegistersMap, 0);

static struct attribute* SKA_MNG_HKREGS_sysfsControl_attributes[] = {
		(struct attribute*)&dev_attr_ListRegistersMap,
		(struct attribute*)&dev_attr_DumpRegistersMap,
		NULL
};

static const struct attribute_group SKA_MNG_HKREGS_sysfsControl_group = {
		.name = "control",
		.attrs = SKA_MNG_HKREGS_sysfsControl_attributes,
};
		...
		struct platform_device* pdev)
{
	...
	ret = sysfs_create_group(&pdev->dev.kobj, &SKA_MNG_HKREGS_sysfs_group);
	if (ret < 0) {
		dev_err(&pdev->dev, "unable to create %s sysfs group\n", SKA_MNG_HKREGS_sysfs_group.name);
		goto out_error;
	}

	ret = sysfs_create_group(&pdev->dev.kobj, &SKA_MNG_HKREGS_sysfsControl_group);
	if (ret < 0) {
		dev_err(&pdev->dev, "unable to create %s sysfs group\n", SKA_MNG_HKREGS_sysfsControl_group.name);
		return ret;
	}
		...
		struct platform_device* pdev)
{
	...
	sysfs_remove_group(&pdev->dev.kobj, &SKA_MNG_HKREGS_sysfs_group);
	sysfs_remove_group(&pdev->dev.kobj, &SKA_MNG_HKREGS_sysfsControl_group);
...
static const struct of_device_id SKA_MNG_HKREGS_id_table[] = {
...
module_platform_driver(SKA_MNG_HKREGS_driver);

MODULE_AUTHOR("Cristian Albanese, Sanitas EG");
MODULE_DESCRIPTION("Sanitas Ska Management HouseKeeping Register Driver");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0");
