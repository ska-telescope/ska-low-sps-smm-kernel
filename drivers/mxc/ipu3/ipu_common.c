				...
				uint32_t u, uint32_t v)
{
	...
	printk(KERN_DEBUG "ipu_init_channel_buffer %08X %08X\n", channel, type);
	printk(KERN_DEBUG "ipu_init_channel_buffer %08X %d %d %d %d\n", pixel_fmt, width, height, stride, rot_mode);
				...
				ipu_buffer_t type, uint32_t bufNum, dma_addr_t phyaddr)
{
	...
	printk(KERN_DEBUG "ipu_update_channel_buffer\n");

	
	if (dma_chan == IDMA_CHAN_INVALID) {
		printk(KERN_DEBUG "ipu_update_channel_buffer dma_chan == IDMA_CHAN_INVALID\n");
		return -EINVAL;
	}
	...
	if ((reg & idma_mask(dma_chan)) == 0) {
		_ipu_ch_param_set_buffer(ipu, dma_chan, bufNum, phyaddr);
	  	...
	  	printk(KERN_DEBUG "ipu_update_channel_buffer (reg & idma_mask(dma_chan)) != 0\n");
		ret = -EACCES;
	}
			...
			ipu_buffer_t type, uint32_t bufNum)
{
	...
	if (dma_chan == IDMA_CHAN_INVALID) {
		printk(KERN_DEBUG "ipu_select_buffer dma_chan == IDMA_CHAN_INVALID\n");
		return -EINVAL;
	}
	spin_lock_irqsave(&ipu->rdy_reg_spin_lock, lock_flags);
